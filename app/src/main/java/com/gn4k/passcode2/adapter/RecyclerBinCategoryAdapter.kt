...
class RecyclerBinCategoryAdapter(private val categoryList: List<String>, private val context: Context) :
    RecyclerView.Adapter<RecyclerBinCategoryAdapter.CategoryViewHolder>() {
        ...
        reference.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (itemSnapshot in snapshot.children) {
                    ...
                    if (passData != null) {
                        list.add(passData)
                        Other.allPassList.add(passData)
                        itemSnapshot.key?.let { keylist.add(it) }
                        itemSnapshot.key?.let { Other.allPassKey.add(it) }
                        Other.allCategory.add(catogery)
                    }
                }


                val layoutManager = LinearLayoutManager(context)
                val adapter = RecyclerBinPassDataAdapter(catogery, keylist, list, context)
